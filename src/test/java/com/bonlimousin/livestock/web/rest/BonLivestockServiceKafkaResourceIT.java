...
import com.bonlimousin.livestock.config.KafkaProperties;
...
class BonLivestockServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        BonLivestockServiceKafkaResource kafkaResource = new BonLivestockServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bon-livestock-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/bon-livestock-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
